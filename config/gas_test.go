...
func TestRandomGasPickerStreamSeeds(t *testing.T) {
	...
	seededA := drawN(t, randomPicker(t, 20000, 30000), newTestRng(seed), n)
	seededB := drawN(t, randomPicker(t, 20000, 30000), newTestRng(seed), n)
...
func TestGenerateGasForFixedAndEmpty(t *testing.T) {
	...
	gas, err := fixed.GenerateGas(newTestRng(1))
	require.NoError(t, err)
	require.Equal(t, uint64(21000), gas)

	var empty config.GasPicker
	require.NoError(t, empty.UnmarshalJSON([]byte(`{}`)))
	gas, err = empty.GenerateGas(newTestRng(1))
	require.NoError(t, err)
	require.Zero(t, gas)
}
