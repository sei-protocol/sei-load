...
- **Run scope rides on the Resource, never per-sample labels.** `run_id`, `commit_id`, `chain_id`, `workload`, `service.instance.id` are process-lifetime constants. Putting them on every metric sample would multiply cardinality by (runs × endpoints × buckets). Prometheus joins them in via `target_info{run_id=...}`.
